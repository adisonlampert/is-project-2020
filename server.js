...
app.post("/addOpps", (request, response) => {
  ...
  if (!process.env.DISALLOW_WRITE) {
    db.run(
      "INSERT INTO Pending (name, category, type, country, state, city, startDate, endDate, deadline, cost, description, link, visibility) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
      request.body.name,
...
app.use(protectPath(/^\/protected\/.*$/));
 ...
 app.get('/admin', function(request, response) {
 	if (request.session.loggedin) {
 		return response.sendFile(__dirname + '/protected/admin.html');
 	} else {
